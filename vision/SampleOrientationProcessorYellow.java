...
public class SampleOrientationProcessorYellow implements VisionProcessor {
    ...
    public RectDrawer.SampleColor colorType = RectDrawer.SampleColor.YELLOW;

    private ArrayList<RotatedRect> rects;

    public SampleOrientationProcessorYellow(Telemetry telemetry) {
    ...
    public void setColorType(RectDrawer.SampleColor color) {
        colorType = color;
    }
...
//        Imgproc.dilate(inRange, inRange, kernel2);
        ...
        if (!filteredRects.isEmpty()) {
            ...
            // Based on the converted angle between -90 and 90
            // Uses linear piecewise interpolation
            if (sampleAngle >= -90 && sampleAngle <= -45) {
                servoPosition = 0.5 + (0.25 - 0.5) * (sampleAngle + 90) / 45; // 0.5 to 0.25
            } else if (sampleAngle > -45 && sampleAngle <= 0) {
                servoPosition = 0.25 + (1 - 0.25) * (sampleAngle + 45) / 45; // 0.25 to 1
            } else if (sampleAngle > 0 && sampleAngle <= 45) {
                servoPosition = 1 - (1 - 0.75) * sampleAngle / 45; // 1 to 0.75
            } else if (sampleAngle > 45 && sampleAngle <= 90) {
                servoPosition = 0.75 + (0.5 - 0.75) * (sampleAngle - 45) / 45; // 0.75 to 0.5
            }
